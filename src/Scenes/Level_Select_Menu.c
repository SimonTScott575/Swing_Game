...
void Load_Level_Select_Menu(geScene* scene) {
  ...
  Level_Select_Camera* lsc = Create_Level_Select_Camera(40, scene);

  Build_Level_Basics_Load(g_level_specs[0].offset_x, scene, &l1_builder._super);
  Build_Level_Basics_Load(g_level_specs[1].offset_x, scene, &l7_builder._super);
  Build_Level_Basics_Load(g_level_specs[2].offset_x, scene, &l2_builder._super);
  Build_Level_Basics_Load(g_level_specs[3].offset_x, scene, &l3_builder._super);
  Build_Level_Basics_Load(g_level_specs[4].offset_x, scene, &l4_builder._super);
  Build_Level_Basics_Load(g_level_specs[5].offset_x, scene, &l5_builder._super);
  Build_Level_Basics_Load(g_level_specs[6].offset_x, scene, &l6_builder._super);
  Build_Level_Basics_Load(g_level_specs[7].offset_x, scene, &l8_builder._super);

  grRendering_System2D* rs = grCreate_Rendering_System2D(&lsc->camera2D);

  UI_Camera* ui_camera = Create_UI_Camera(scene);

  Menu_Button* prev_button       = Create_Menu_Button(-1,0,
                                                      0.15,0,
                                                      0.1,0.3,
                                                      "../Resources/Textures/left_arrow_white_256.png",
                                                      On_Click_Prev_Level,
                                                      scene);
  Menu_Button* next_button       = Create_Menu_Button(1,0,
                                                      -0.15,0,
                                                      0.1,0.3,
                                                      "../Resources/Textures/right_arrow_white_256.png",
                                                      On_Click_Next_Level,
                                                      scene);
  Menu_Button* play_level_button = Create_Menu_Button(0,-1,
                                                      0,0.2,
                                                      0.5,0.2,
                                                      "../Resources/Textures/clear_1.png",
                                                      On_Click_Play_Level,
                                                      scene);
  Menu_Button* main_menu_button  = Create_Menu_Button(-1,-1,
                                                      0.4,0.2,
                                                      0.5,0.2,
                                                      "../Resources/Textures/clear_1.png",
                                                      On_Click_Main_Menu,
  ...
  Menu_Text* play_level_text = Create_Menu_Text("START", NULL, 0,-1,  0,  0.15,  0.6, scene);
  Menu_Text* main_menu_text  = Create_Menu_Text("MENU", NULL, -1,-1,  0.4,0.15,  0.6, scene);

  Menu_Text* level_name_text = Create_Menu_Text("LEVEL_TITLE", "../Resources/Fonts/Fira/FiraSans-HeavyItalic.ttf",-1, 1, 0.1,-0.25, 1, scene);
  Level_Title* level_title = Level_Title_new(-15, level_name_text->text_r);
  ...
  Menu_Text* best_time_text = Create_Menu_Text("BEST TIME:", NULL, 1, 1,-0.1,-0.25, 0.6, scene);
  Menu_Text* par_time_text  = Create_Menu_Text("PAR:",       NULL, 1,-1,-0.1, 0.35, 0.6, scene);

  Display_Time* dt_par = Display_Time_new("PAR: ", par_times, par_time_text->text_r, 15);
  geAdd_Component(&dt_par->_super, &par_time_text->_super);

  Display_Time* dt_best = Display_Time_new("BEST: ", best_times, best_time_text->text_r, 15);
  ...
  Menu_Text* locked_text = Create_Menu_Text("BEAT PAR TO CONTINUE", NULL, 0, 0,0,0, 0.5, scene);
