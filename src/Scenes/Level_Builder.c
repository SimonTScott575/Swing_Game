...
void Build_Level(float x_offset, Level_Builder* builder, geScene* scene) {
  ...
  Build_Level_Basics_Load(x_offset, scene, builder);
  ...
  if (builder->as_scene) {
    ...
    camera = Create_Camera(&builder->player->frame, 20, scene);

    rope = Create_Rope(&builder->player->rb, scene);

    //
    Load_Entities_Level_UI_Build(scene, &level_ui_build);

    Load_Entities_End_Level_Menu_Build(&builder->portal->pc, &level_ui_build.timer->clock, &builder->player->pc, scene, &end_level_menu_build);
