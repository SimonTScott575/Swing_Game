...
void Load_Entities_Level_UI_Build(geScene* scene, Level_UI_Build* build) {

  //
  build->ui_camera = Create_UI_Camera(scene);

  build->timer_sec_mt = Create_Menu_Text("0", NULL,    1,1,-0.4,-0.2, 0.8, scene);
  build->timer_msec_mt = Create_Menu_Text(".00", NULL, 1,1,-0.4,-0.2, 0.8, scene);

  build->score_num_mt = Create_Menu_Text("", NULL, -1,1, 0.25,-0.2, 0.8, scene);
  build->score_den_mt = Create_Menu_Text("", NULL, -1,1, 0.25,-0.2, 0.8, scene);
