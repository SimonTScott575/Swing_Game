...
int main() {
  ...
  mFrame2D frame2 = init_mFrame2D(mMul_f_V2f( 1,mVector2f_Y), 0, mMul_f_V2f(0.5, mVector2f_ONE));
  mFrame2D frame3 = init_mFrame2D(mMul_f_V2f(0.5,mVector2f_Y), 0, mMul_f_V2f(0.4, mVector2f_ONE));
  ...
  //frame3.position = mAdd_V2f(frame3.position, mMul_f_V2f(0, mVector2f_X));

  //float vertices1[8] = {0.5,0.5, -0.5,0.5, -0.5,-0.5, 0.5,-0.5};
  phCollider2D* collider1 = new_phCollider2D_AABB(&frame1, 1, 1);
  phCollider2D* collider2 = new_phCollider2D_AABB(&frame2, 1, 1);
  phCollider2D* collider3 = new_phCollider2D_AABB(&frame3, 1, 1);
  ...
  phRigid_Body2D rb2 = init_phRigid_Body2D(&frame2, 4, 2/12.0f*(0.2*0.2*2), collider2);
  rb2.velocity = mMul_f_V2f( 0, mVector2f_Y);
  //rb2.angular_velocity = 3;
  phRigid_Body2D rb3 = init_phRigid_Body2D(&frame3, 2, 2/12.0f*(0.2*0.2*2), collider3);
  //rb3.velocity = mMul_f_V2f( -0.1, mVector2f_Y);
  ...
  // dAppend_LLRB(&rb1, rb_sys->_rigid_bodies2);
  // dAppend_LLRB(&rb2, rb_sys->_rigid_bodies2);
  // dAppend_LLRB(&rb3, rb_sys->_rigid_bodies2);

  rb_sys->global_force = mMul_f_V2f(-0.2, mVector2f_Y);
