...
//
//   // phCollider2D* c = rb_node->element->collider;
//   phAABB_Collider2D* g = c->_sub;
//
//   uint64_t n_collisions = 0; // bool is_colliding = false;
//
//   mVector2f norm_dir = direction;
//   if (!mNormalize_V2f(&norm_dir)) { return false; }
//   mVector2f norm_dir_perp = (mVector2f){{-norm_dir.i[1], norm_dir.i[0]}};
//
//   float closest_distance = INFINITY;
...
//
//     // positions of line segment ends
//     mVector2f pos1 = mAdd_V2f(
//       c->frame->position,
//       (mVector2f){{ signs1_X[i] * c->frame->scale.i[0]*g->X_length/2, signs1_Y[i] * c->frame->scale.i[1]*g->Y_length/2}}
//     );
//     mVector2f pos2 = mAdd_V2f(
//       c->frame->position,
//       (mVector2f){{ signs2_X[i] * c->frame->scale.i[0]*g->X_length/2, signs2_Y[i] * c->frame->scale.i[1]*g->Y_length/2}}
//     );
//
//     // positions relative to ray position
//     mVector2f rel_pos1 = mSub_V2f(pos1, position);
//     mVector2f rel_pos2 = mSub_V2f(pos2, position);
//
//     if (
//       mCross_V2f(rel_pos1,direction)*mCross_V2f(rel_pos2,direction) < 0
//     ) {
//
//       // signed distances along direction
//       float distance1 = mDot_V2f(rel_pos1, norm_dir);
//       float distance2 = mDot_V2f(rel_pos2, norm_dir);
//
//       // signed distances along perpendicular to direction
//       float distance1_perp = mDot_V2f(norm_dir_perp, rel_pos1);
//       float distance2_perp = mDot_V2f(norm_dir_perp, rel_pos2);
//
//       // if (approx) inside edge dont trigger
//       if (fabs(distance1_perp) + fabs(distance2_perp) < 0.001) {
//         continue;
//       }
//
//       float gradient = ( distance1 - distance2 ) / ( fabs(distance1_perp) + fabs(distance2_perp) );
//       float min_distance = fabs(distance1) < fabs(distance2) ? fabs(distance1) : fabs(distance2);
//
//       mVector2f this_position = mAdd_V2f(
//         position,
//         mMul_f_V2f(distance2, norm_dir)
//       );
//       this_position = mAdd_V2f(
//         this_position,
//         mMul_f_V2f(gradient*fabs(distance2_perp), norm_dir)
//       );
//
//       // if this_position behind or at position, don't trigger yet
//       if ( mDot_V2f(mSub_V2f(this_position,position), direction) <= 0 ) {
//         continue;
//       } else {
//
//         // trigger !
...
//
//           closest_position = this_position;
//           closest_distance = mNorm_V2f(mSub_V2f(closest_position, position));
//           closest_normal = normals[i];
//
//         }
//
...
// uint64_t phGet_Collider_Collisions_Count(phCollider2D* collider) {
//
//   if (collider->save_collisions) {
//     return collider->_collisions_length;
//   }
//
//   return 0;
//
// }
// phCollision2D* phGet_Collider_Collisions(phCollider2D* collider) {
//
//   if (collider->save_collisions) {
//     return collider->_collisions;
//   }
//
//   return NULL;
//
