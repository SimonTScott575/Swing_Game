#error NOT FINISHED!
//!!! remove all glfw/glad
//??? but keep glfw ?
//TODO; incorporate screen
...
int main() {
  ...
  grInit(WINDOW_SIZE_X, WINDOW_SIZE_Y);
  ...
  grCamera2D* camera = grCreate_Camera_2D(&camera_frame, 2, 2);
  ...
  grDestroy_Texture(texture);
  grDestroy_Model(model);
  grDestroy_Shader(shader);
