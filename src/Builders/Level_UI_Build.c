...
void Load_Entities_Level_UI_Build(Level_UI_Build* build) {
  ...
  build->timer_sec_mt = Create_Menu_Text("0",    1,1,-0.4,-0.2, 1);
  build->timer_msec_mt = Create_Menu_Text(".00", 1,1,-0.4,-0.2, 1);

  build->score_num_mt = Create_Menu_Text("", -1,1, 0.2,-0.2, 1);
  build->score_den_mt = Create_Menu_Text("", -1,1, 0.2,-0.2, 1);
