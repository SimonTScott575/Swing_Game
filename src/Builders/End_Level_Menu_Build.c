...
void Load_Entities_End_Level_Menu_Build(Portal_Catcher* pc, Timer_Clock* clock, Player_Controller* player_controller, End_Level_Menu_Build* build) {
  ...
  build->cd_menu_text = Create_Menu_Text_With_Resolution("3", 0,0,0,-0.15, 1.5, 512);

  build->back_mt       = Create_Menu_Text("MENU",    -1,-1,  0.4,0.15, 0.7);
  build->restart_mt    = Create_Menu_Text("RESTART",  0,-1,  0,  0.15, 0.7);
  build->next_level_mt = Create_Menu_Text("NEXT",     1,-1, -0.4,0.15, 0.7);
